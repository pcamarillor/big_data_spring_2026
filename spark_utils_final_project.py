...
class SparkUtils:
    ...
    def __init__(self, app_name, master_url=None, spark_jars=None, spark_packages=None, mongodb_connector=None):
        spark_builder = (
        ...
        if spark_packages is not None:
            if isinstance(spark_packages, list):
                packages.extend(spark_packages)
            else:
                packages.append(spark_packages)

        if mongodb_connector is not None:
            if isinstance(mongodb_connector, list):
                packages.extend(mongodb_connector)
            else:
                packages.append(mongodb_connector)
